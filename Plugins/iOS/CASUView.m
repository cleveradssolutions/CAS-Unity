...
@implementation CASUView {
    ...
    /// In landscape apps, it tries to force the orientation to be portrait, causing the safe areas
    /// insets to switch to portrait mode insets even though the SKProductViewController looks like
    /// it's presenting in landscape. As woraround we ignore Top safearea offset if Portrait
    /// Orientation is not Supported.
    BOOL _isPortraitOrientationSupported;
}

- (instancetype)initWithCASID:(NSString *)casID
                    forClient:(CASViewClientRef _Nullable *)adViewClient
                         size:(int)size {
    self = [super init];
...
- (void)enable {
    ...
    [self.bannerView addObserver:self
                      forKeyPath:@"center"
    ...
    self.bannerView.isAutoloadEnabled =
        [CAS.settings getLoadingMode] != CASLoadingManagerModeManual;
    ...
    if (_isPortraitOrientationSupported &&
        ...
        [NSLayoutConstraint
            constraintWithItem:self.bannerView
                     ...
                     relatedBy:NSLayoutRelationGreaterThanOrEqual
                        toItem:_isPortraitOrientationSupported ? safeArea : unityView
                     attribute:NSLayoutAttributeTop
                    multiplier:1.0
                      constant:0.0],
...
- (void)orientationChangedNotification:(NSNotification *)notification {
    ...
    if (UIDeviceOrientationIsValidInterfaceOrientation([[UIDevice currentDevice] orientation])) {
        if (_activeSizeId == kCASUSize_ADAPTIVE || _activeSizeId == kCASUSize_FULL_WIDTH ||
            _activeSizeId == kCASUSize_LINE) {
...
- (void)refreshPixelsRect:(CGRect)rect {
    ...
    if (_didResignActive) {
        ...
        // destroyed. We shall not call unity API, and definitely not script callbacks, so nothing
        // to do here
        return;
    }
    ...
    if (self.rectCallback) {
        ...
        self.rectCallback(self.client, CGRectGetMinX(rect) * scale, CGRectGetMinY(rect) * scale,
...
- (CASSize *)getSizeByCode:(int)sizeId {
    ...
    case kCASUSize_SMART:
        return [CASSize getSmartBanner];

    case kCASUSize_LEADER:
...
- (void)bannerAdView:(CASBannerView *)adView didFailWith:(CASError *)error {
    if (self.actionCallback) {
        self.actionCallback(self.client, kCASUAction_FAILED, (int)error.code,
                            CASUStringToUnity(error.description));
    }
}
...
/// Attention. NSLayoutConstraint with `[self getWindow].safeAreaLayoutGuide` can be lost (removed)
/// after Window refreshed (Interstitial ad), use `superview.safeAreaLayoutGuide` instead.
- (void)refreshPosition {
    ...
    case kCASUPosition_TOP_RIGHT: {
        self.constraintY = [NSLayoutConstraint
            constraintWithItem:self.bannerView
                     attribute:NSLayoutAttributeTop
                     relatedBy:NSLayoutRelationEqual
                        toItem:_isPortraitOrientationSupported ? safeArea : superView
                     attribute:NSLayoutAttributeTop
                    multiplier:1.0
                      constant:_verticalOffset];
        break;
    }
    ...
    case kCASUPosition_BOTTOM_RIGHT: {
        self.constraintY = [NSLayoutConstraint constraintWithItem:self.bannerView
                                                        attribute:NSLayoutAttributeBottom
                                                        relatedBy:NSLayoutRelationEqual
                                                           toItem:safeArea
                                                        attribute:NSLayoutAttributeBottom
    ...
    case kCASUPosition_MIDDLE_LEFT:
        self.constraintX = [NSLayoutConstraint constraintWithItem:self.bannerView
                                                        attribute:NSLayoutAttributeLeft
                                                        relatedBy:NSLayoutRelationEqual
                                                           toItem:safeArea
                                                        attribute:NSLayoutAttributeLeft
                                                       multiplier:1.0
                                                         constant:_horizontalOffset];
        break;
